...
`,
   ...
   */
  async passLogin(username, password = this.password, mfaCode) {
    try {
    ...
    console.log(data);
    if(!data.token && !data.ticket && !data.mfa) return { error: 'LOGIN_FAILED', message: 'Login failed because of password', loggedIn: false, token: null};
    this.password = password;
    if (!data.token && data.ticket && data.mfa) {
      if(!mfaCode) return { error: 'LOGIN_FAILED_2FA', message: '2FA Code is required', loggedIn: false, token: null};
      const normal2fa = /(\d{6})/g;
      const backupCode = /([a-z0-9]{4})-([a-z0-9]{4})/g;
      if (!mfaCode || typeof mfaCode !== 'string') {
        return { error: 'LOGIN_FAILED_2FA', message: '2FA Code is invalid', loggedIn: false, token: null};
      }
      if (normal2fa.test(mfaCode) || backupCode.test(mfaCode)) {
        const data2 = await this.api.auth.mfa.totp.post({
         ...
         this.login(data2.token);
         return { error: null, message: 'Logged in', loggedIn: true, token: data2.token};
      } else {
        return { error: 'LOGIN_FAILED_2FA', message: '2FA Code is invalid', loggedIn: false, token: null};
      }
    } else if (data.token) {
       this.login(data.token);
        return { error: null, message: 'Logged in', loggedIn: true, token: data.token};
    } else {
      return { error: 'LOGIN_FAILED', message: 'Login failed', loggedIn: false, token: null};
    }
  } catch (error) {
    return { error: 'LOGIN_FAILED', message: 'Login failed', loggedIn: false, token: null};
  }
   ...
   */
  QRLogin(options = {}) {
    const QR = new DiscordAuthWebsocket({ ...options, autoLogin: true, Client: this });
